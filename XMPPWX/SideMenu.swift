...
class SideMenu : NSObject, MenuTableViewControllerDelegate {
    ...
    let menuWidth : CGFloat = 140.0
    let menuTableViewTopInset : CGFloat = 0.0 // if you use translusent navigation bar
    ...
    func setupMenuView() {
        ...
        sideMenuContainerView.frame = CGRectMake(-menuWidth-1.0, sourceView.frame.origin.y, menuWidth, sourceView.frame.height)
    ...
    func toggleMenu (shouldOpen: Bool) {
        ...
        let gravityDirectionX: CGFloat = (shouldOpen) ? 0.6 : -0.6;
    ...
    func toggleMenu () {
        if (isMenuOpen) {
            ...
            toggleMenu(true)
            sideMenuContainerView.layer.shadowOffset = CGSizeMake(4.0, 4.0);
        }
    }
}